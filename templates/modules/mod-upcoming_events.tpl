...
{if count($modUpcomingEvents)}
	...
	<div class="row">
		{section name=ix loop=$modUpcomingEvents}
			...
			<!--{if !$smarty.section.ix.first}</div></tr>{/if}-->
			<div class="col-md-3">
			{if $nonums != 'y'}
				<div class="module event-num">{$smarty.section.ix.index_next}</div>
			{/if}
			<div class="module vevent"{if $showColor eq 'y' and $infocals.$calendarId.custombgcolor ne ''} style="background-color:#{$infocals.$calendarId.custombgcolor}"{/if}>
				...
				{if $modUpcomingEvents[ix].allday}
					<span class="dtstart" title="{$modUpcomingEvents[ix].start|isodate}">{$modUpcomingEvents[ix].start|tiki_short_date}</span>
					{else}
						<span class="dtstart" title="{$modUpcomingEvents[ix].start|isodate}">{$modUpcomingEvents[ix].start|tiki_date_format:$date_format}</span>
					{if $showEnd eq 'y'}
																							            -
						<span class="dtend" title="{$modUpcomingEvents[ix].end|isodate}">{if $module_params.date_format}{$modUpcomingEvents[ix].end|tiki_date_format:$date_format}{elseif $modUpcomingEvents[ix].start|tiki_short_date ne $modUpcomingEvents[ix].end|tiki_short_date}{$modUpcomingEvents[ix].end|tiki_short_datetime}{else}{$modUpcomingEvents[ix].end|tiki_short_time}{/if}</span>
					{/if}
				{/if}
				</div>
